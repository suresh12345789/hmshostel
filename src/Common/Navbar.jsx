...
//             smooth
//             to="#Download"
//             className={`group  text-white rounded-3xl px-5  border-2 border-transparent bg-clip-padding 
//     hover:border-gradient-to-r border-orange-500  py-2 w-max duration-700 ${isActive(
//               "#Download"
//             )}`}
...
const Navbar = () => {
  ...
  return (
    <header
      className={`shadow-lg w-full z-10 sticky top-0 transition-all duration-300 ${isScrolled ? "bg-[#001324]" : "bg-transparent"}`}
    >
      <div className="flex items-center justify-between md:py-4 py-2 px-5 md:px-10">
        <div>
          <a href="/">
        ...
        <nav className="items-center hidden lg:space-x-12 text-white space-x-4 md:flex text-lg ">
          ...
          <HashLink
            smooth
            to="#Contact"
            className={`group w-max bg-customRed rounded-lg ${isActive("#Contact")}`}
            onClick={() => handleScroll("#Contact")}
          >
            <h1 className="hover:border-b-2 border-orange-400">Contact Us</h1>
          </HashLink>
          ...
          <HashLink
            smooth
            to="#Download"
            className={`group text-white rounded-3xl px-5 border-2 border-transparent bg-clip-padding hover:border-gradient-to-r border-orange-500 py-2 w-max duration-700 ${isActive("#Download")}`}
            onClick={() => handleScroll("#Download")}
          >
            Book a Demo
          </HashLink>
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 bg-opacity-50 z-50 text-blue-400 font-semibold text-xl transition-transform transform ${isMenuOpen ? "translate-x-0 duration-700" : "translate-x-full duration-700"} md:hidden`}
        onClick={() => setIsMenuOpen(false)}
      >
        <div
          className="absolute right-0 w-full h-full bg-white text-center p-5 transition-transform transform shadow-lg bg-darkGrey text-tan"
        ...
        >
          ...
          <nav className="mt-8 space-y-10 text-lg flex flex-col items-center">
            {/* Home link now uses <a href="/"> */}
            <a href="/" className={`${isActive("#")}`} onClick={() => setIsMenuOpen(false)}>
              Home
            </a>
            ...
            <HashLink
              smooth
              to="#Download"
              className={`group bg-orange-500 text-white rounded-3xl px-5 py-3 w-max duration-700 ${isActive("#Download")}`}
              onClick={() => handleScroll("#Download")}
            >
              Book a Demo
            </HashLink>
